...
@Component({
  selector: "ngx-validation-credit-details",
  templateUrl: "./validation-credit-details.component.html",
  styleUrls: ["./validation-credit-details.component.scss"],
})
export class ValidationCreditDetailsComponent implements OnInit {
  inputData: any;
  tierData: any;
  accountData: any;
  ...
  constructor(
    ...
    private dialogRef: NbDialogRef<ValidationCreditDetailsComponent>
  ) {}
  ...
  ngOnInit(): void {
    ...
    this.accountService
      ...
      .subscribe((response) => {
        ...
        this.registerForm1.controls["solde"].setValue(
          this.accountData[0].solde
        );
        this.registerForm1.controls["numCpt"].setValue(
          this.accountData[0].numCpt
        );
        this.registerForm1.controls["active"].setValue(
          this.accountData[0].active
        );
        this.registerForm1.controls["typeAccount"].setValue(
          this.accountData[0].typeAccount
        );
        this.registerForm1.controls["currency"].setValue(
          this.accountData[0].currency.name
        );
      });
  }
  ...
  validationContrat() {
    this.engagementService
      .updateContractbyStatus(this.inputData.id, ContractState.VALIDATED)
      .subscribe((data) => {
        this.toastrService.success("Contrat validé", "Success");
        this.onReset();
      });
  }
  ...
  refusContrat() {
    this.engagementService
      .updateContractbyStatus(this.inputData.id, ContractState.CANCELED)
      .subscribe((data) => {
        this.toastrService.success("Contrat refusé", "Success");
        this.onReset();
      });
  }
